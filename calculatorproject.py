...
def add(value):
    global expression
    expression+=value
    label_reasult.config(text=expression)
def clear():
    global expression
    expression=""
    label_reasult.config(text=expression)
    ...
    if expression!="":
        try:
            result=eval(expression)
                ...
                result="error"
                expression=""
    label_reasult.config(text=result)
    expression=str(result)


label_reasult=tkinter.Label(root,text="")
label_reasult.grid(row=0,column=0,columnspan=4)
...
button_4=tkinter.Button(root,text="4",command=lambda: add("3"))
